...
// Data class defining a parking time slot with its rate
data class TimeSlot(val label: String, val startMin: Int, val endMin: Int, val rate: Double)

// Enum to define how the parking duration is rounded for calculation
enum class RoundingMode {
    Proportionnel,
    HeureEntamee
}

// Data class representing the usage of a specific time slot
data class SlotUsage(val slotLabel: String, val minutes: Int, val cost: Double)

// Data class for holding the final calculation result
data class CalculationResult(val usages: List<SlotUsage>, val totalMinutes: Int, val totalCost: Double)

// Data class for storing an entry in the calculation history
